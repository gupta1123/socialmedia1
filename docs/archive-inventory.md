...
| `skills/image-edit/v1/` | `archive/legacy/skills/image-edit/v1/` | Root image-edit skills duplicated an experimental mask-edit planner tree that is no longer part of the runtime. | Direct AI edit now runs from `apps/api` without a Python planner. |
| `services/socialpython/agents/amenity_tools.py` | `archive/legacy/services/socialpython-prompt-compiler/agents/amenity_tools.py` | Legacy helper from the old prompt compiler stack. The replacement compiler does not use the old amenity helper module. | `services/socialpython/compiler/tools.py` |
| Legacy `services/socialpython/agents/creative_director_notebook.py` implementation | `archive/legacy/services/socialpython-prompt-compiler/agents/creative_director_notebook.py` | The old service-owned prompt compiler implementation was replaced by the new three-stage compiler package. | `services/socialpython/compiler/` |
| Legacy `services/socialpython/skills/prompt/v2/` skill pack | `archive/legacy/services/socialpython-prompt-compiler/skills/prompt/v2/` | The old prompt compiler skill set was replaced by a smaller static skill pack aligned with the new analyst -> crafter -> verifier pipeline. | `services/socialpython/skills/prompt/v2/` |
| Root prototype notebook (`working.ipynb`) | `archive/legacy/services/socialpython-prompt-compiler/notebook/working.ipynb` | Prototype notebook informed the replacement compiler architecture but should not remain in repo root once the server owns the productionized flow. | `services/socialpython/compiler/` |
