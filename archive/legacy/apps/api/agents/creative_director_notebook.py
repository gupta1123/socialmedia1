...
## Strategy Route (MUST name one style family, 0-2 modifiers, one dominant poster archetype, and one text architecture when text behavior matters)
## Risk Checks
...
- Load `style-taxonomy` before writing Strategy Route. Use it to choose one named family and optional modifiers rather than inventing ad hoc style prose.
...
- If `style-taxonomy` is loaded, each variation must resolve one family and lever bundle into concrete image behavior. Do not leave style as vague adjectives.
...
- For poster-led social posts, the final prompt should read like one finished social creative, not a raw hero-image brief with empty-space instructions bolted on.
...
- Always load `style-taxonomy` before composition planning.
...
- Load `copy-typography-planner` when the post includes in-image text, logo, QR, offer, exact copy, or when text architecture is important to the route.
...
def build_agents() -> AgentBundle:
    ...
    brief_analyst = Agent(
        ...
        instructions=[
            ...
            "Before writing Strategy Route, call get_skill_instructions for brief-interpreter, style-taxonomy, and for the exact postTypeContract.playbookKey when present.",
            ...
            "Strategy Route must choose one style family and avoid generic premium-style wording with no visual consequences.",
            ANALYST_SKILL_WORKFLOW_INSTRUCTION,
            ANALYST_OUTPUT_INSTRUCTION,
        ],
    ...
    prompt_crafter_kwargs: dict[str, Any] = {
        ...
        "instructions": [
            ...
            "Resolve every route into one clear poster archetype or scene archetype. Do not let everything collapse into the same tower-plus-space composition.",
